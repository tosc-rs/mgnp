...
mod alloc {
    pub fn channel<T>(bound: usize) -> (Sender<T>, Receiver<T>) {
        ...
        let tx: ManuallyDrop<spitebuf::Sender<Never, Never>> = unsafe { transmute(ManuallyDrop::new(tx)) };
        let rx: ManuallyDrop<spitebuf::Receiver<Never, Never>> = unsafe { transmute(ManuallyDrop::new(rx)) };
    ...
    pub fn deser_channel<T: DeserializeOwned + 'static, STO: spitebuf::Storage<T> + 'static>(bound: usize) -> (Sender<Vec<u8>>, Receiver<T>) {
        let (tx, rx) = mpsc::sync_channel::<T>(bound);
        let tx: ManuallyDrop<spitebuf::Sender<Never, Never>> = unsafe { transmute(ManuallyDrop::new(tx)) };
        let rx: ManuallyDrop<spitebuf::Receiver<Never, Never>> = unsafe { transmute(ManuallyDrop::new(rx)) };
    ...
    pub fn deser_ref_channel<T: DeserializeOwned + 'static, STO: spitebuf::Storage<T> + 'static>(bound: usize) -> (RefSender, Receiver<T>) {
        let (tx, rx) = mpsc::sync_channel::<T>(bound);
        let tx: ManuallyDrop<spitebuf::Sender<Never, Never>> = unsafe { transmute(ManuallyDrop::new(tx)) };
        let rx: ManuallyDrop<spitebuf::Receiver<Never, Never>> = unsafe { transmute(ManuallyDrop::new(rx)) };
...
fn bypass_sender<T: 'static, STO: spitebuf::Storage<T> + 'static>(tx: *const spitebuf::Sender<Never, Never>, t: *const Never) -> SendOutcome {
    ...
    match tx.enqueue_sync(t) {
        ...
        Err(EnqueueError::Full(t)) | Err(EnqueueError::Closed(t)) | Err(EnqueueError::InternalError(t)) => {
            core::mem::forget(t);
            SendOutcome::Untaken
        },
    }
...
fn bypass_receiver<T: 'static, STO: spitebuf::Storage<T> + 'static>(rx: *const spitebuf::Receiver<Never, Never>, t: *mut Never) -> RecvOutcome {
    let rx: &spitebuf::Receiver<T, STO> = unsafe { &*rx.cast() };
...
unsafe fn type_drop_sender<T: 'static, STO: spitebuf::Storage<T> + 'static>(container: &mut ManuallyDrop<spitebuf::Sender<Never, Never>>) {
    let container: &mut ManuallyDrop<spitebuf::Sender<T, STO>> = core::mem::transmute(container);
    ManuallyDrop::drop(container);
}

unsafe fn type_drop_receiver<T: 'static, STO: spitebuf::Storage<T> + 'static>(container: &mut ManuallyDrop<spitebuf::Receiver<Never, Never>>) {
